...
public:
    vector<int> findAllPeople(int n, vector<vector<int>>& meetings,
                              int firstPerson) {
        vector<bool> can(
            n); // Vector to represent whether a person can know the secret
        can[0] = can[firstPerson] =
            true; // Person 0 and the person specified by firstPerson initially
                  // know the secret

        map<int, vector<pair<int, int>>>
            mp; // Map to store meetings grouped by time
        ...
        for (auto& [k, v] : mp) {
            unordered_map<int, vector<int>>
                graph; // Adjacency list to represent the graph of people in the
                       // meeting
            unordered_set<int>
                st; // Set to store people who currently know the secret
